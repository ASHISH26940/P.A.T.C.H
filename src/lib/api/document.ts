...
export async function createCollection(
  collectionData: DocumentCollectionCreate
): Promise<any> {
  try {
...
): Promise<DocumentsAddedResponse> {
  try {
    // The endpoint likely expects a specific payload structure, e.g., { documents: [...] }
    const response = await apiClient.post<DocumentsAddedResponse>(
      `/v1/documents/${collectionName}/documents`,
      { documents }
    );
    return response.data;
  } catch (error) {
 ...
 */
export async function queryDocumentsInCollection(
  collectionName: string,
  query: DocumentQuery
): Promise<DocumentQueryResult[]> {
  try {
    const response = await apiClient.post<DocumentQueryResult[]>(
      `/v1/documents/${collectionName}/query`,
      query
    );
    return response.data;
  } catch (error) {
